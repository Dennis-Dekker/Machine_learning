...
def roc_plot(X_sm_train,y_sm_train,X_test, y_test, linear, tree, knn):
    """
    ...
    y_sm_train=label_binarize(y_sm_train,classes=[0, 1, 2, 3, 4])
    n_classes = 5
    ...
    #KNN roc plot
    y_score_knn = classifier_knn.fit(X_sm_train, y_sm_train).predict_proba(X_test)
    ...
    y_score_dt = classifier_dt.fit(X_sm_train, y_sm_train).predict_proba(X_test)
    #compute ROC curve for each class
    fpr=dict()
    tpr=dict()
    roc_auc=dict()
...
def decision_tree(X_sm_train, X_test, y_sm_train, y_test):
    tree=DecisionTreeClassifier()
    dtree_model = tree.fit(X_sm_train, y_sm_train) 
...
def support_vector_machine(X_sm_train, X_test, y_sm_train, y_test):
     # training a linear SVM classifier 
    linear=SVC(kernel = 'linear', C = 1)
    svm_model_linear = linear.fit(X_sm_train, y_sm_train) 
...
def k_nearest_neighbors(X_sm_train, X_test, y_sm_train, y_test):
...
################################Correct imbalance#################################################################  
    #Correct imbalance by adding more sintetic samples of underrepresent classes.
...
##################################################################################################################
    ...
    cm_dt, acc_dt,tree=decision_tree(X_sm_train, X_test, y_sm_train, y_test)
    plot_accuracy("decision tree", cm_dt, acc_dt)   

    #SVM
    cm_svm, accuracy_svm, svm_model =support_vector_machine(X_sm_train, X_test, y_sm_train, y_test)
    ...
    roc_plot(X_sm_train, y_sm_train, X_test, y_test, svm_model, tree, knn)
